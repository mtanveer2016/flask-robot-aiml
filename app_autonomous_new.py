...
autonomous_active = False  # This will be set to True when in any autonomous mode
...
class PathPlanner:
    ...
    def get_navigation_commands(self):
        ...
        return base_speed - turn, base_speed + turn
...
def set_mode():
    global current_mode, autonomous_active, ball_follower
    data = request.json
    mode = data.get("mode")
    ...
    if mode in ["manual", "ball_follow", "waypoint_nav", "patrol", "obstacle_avoid"]:
        ...
        # IMPORTANT FIX: Set autonomous_active TRUE for ALL autonomous modes
        if mode == "manual":
            autonomous_active = False
            PWM.set_motor_model(0, 0, 0, 0)
        else:
            autonomous_active = True  # <-- THIS IS THE KEY FIX!
            if mode == "ball_follow":
                ball_follower = SmartBallFollower()  # Reset follower with fresh state
                print("Ball following mode activated - motors will now search for ball")
...
# ================= Video Streaming =================
def generate_frames():
    global current_mode, ball_follower, autonomous_active
    ...
    while True:
        ...
        if current_mode == "ball_follow":
            ...
            # IMPORTANT FIX: Always send motor commands when in ball_follow mode
            # regardless of autonomous_active flag
...
if __name__ == "__main__":
    print("=" * 60)
    print("ðŸ¤– Autonomous Robot Car System with Enhanced Ball Following")
    print("=" * 60)
    print("Access URLs:")
    ...
    print("\nAvailable Modes:")
    print("  ðŸŽ® Manual Control - Drive with keyboard/mouse")
    print("  âš½ Ball Following - Track and follow orange ball (IMPROVED!)")
    print("  ðŸ“ Waypoint Navigation - Follow predefined path")
    print("  ðŸš“ Patrol Mission - Autonomous patrol route")
    print("  ðŸ” Ball Hunt Mission - Search for orange ball")
    print("\nHardware Status:")
    print(f"  ðŸ“¹ Camera: {'Available' if camera_available else 'Not Available'}")
    print(f"  ðŸš— Motors: OK")
    print(f"  ðŸ”Š Buzzer: OK")
    print(f"  ðŸ’¡ LED: OK")
    print("=" * 60)
    print("\nðŸ’¡ TIP: If ball following is not smooth, visit /calibrate to tune settings!")
    print("=" * 60)
